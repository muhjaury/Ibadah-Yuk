	...
	<div class="container">
		<div class="row">
			<div class="col-lg-7 col-xl-8">
				<h5 class="text-uppercase mb-0">Tambah Pengingat</h5>
				<hr class="color-9">
				<div class="row align-items-center">
					<div class="col">
						<form action="<?php echo base_url();?>add/addPengingat" method="post" enctype="multipart/form-data">
							<div class="form-row">
								<input class="form-control mb-3" type="text" placeholder="Nama" name="namapengingat">
								<input class="form-control mb-3" type="date" placeholder="Tanggal" name="tanggal">
								<input class="form-control mb-3" type="time" placeholder="Waktu Ibadah" name="waktu">
								<input class="form-control mb-3" type="text" placeholder="Keterangan" name="deskripsi">
								<select class="form-group form-control mb-3" name="pengingat">
									<?php 
					                    foreach ($data_tipepengingat as $row){
					                        echo '
													<option>'.$row['nama_tipe'].'</option>
					                    ';}
					                ?>
								</select>
								<button class="btn-block btn btn-primary" type="submit">Tambahkan</button>
							</div>
						</form>
					</div>
				</div>
					...
					<div class="col-md- col-lg-12 mb-7">
						<div class="row font-1">
							<div class="col-12 mb-4">
								<h5 class="text-uppercase mb-0">Detail Ibadah</h5>
								</br>
								<table>
								<?php 
					                    foreach ($data_tipepengingat as $row){
					                        echo '
														<tr>
															<td><h6 class="mb-0">'.$row['nama_tipe'].'</h6><td>
															<td><h6 class="mb-0"> Pukul '.$row['waktu'].'</h6><td>
														</tr>
					                    ';}
					                ?>
								</table>
